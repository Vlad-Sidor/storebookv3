...
function Index() {
  ...
  const indicesToShow = [1, 2];

  // Фильтрация titlesData для отображения только книг с нужными индексами
  const filteredTitlesData = useMemo(() => {
    return titlesData?.filter((_, index) => indicesToShow.includes(index + 1)) || [];
  }, [titlesData, indicesToShow]);
  ...
  return (
    ...
    >
      {filteredTitlesData &&
      status === "success" &&
      filteredTitlesData[filteredTitlesData.length - 1]?.status === "success" ? (
        
        filteredTitlesData.map((el, index) => (
